...
import { stopCamera } from "./Components/heplerFunctions/startCam.js"
import { startPeerConnection, handleAnswer } from "./Components/heplerFunctions/peerConnection.js"
...
const Messages = () => {
    ...
    const receiverStreamRef = useRef()
    ...
    useEffect(() => {
        ...
        if (socket) {
            ...
            {/*socket.on("callReceived", (data) => {
                setIsCalling(true)
                const { pp, name, id } = data
                setCallReceiver({ pp, name, id })
                startPeerConnection(socketRef.current, receiverStreamRef)
                    .then(stream => {
    ...
    function handleCallEnd(from) {
        // dont you dare touch this this is the receiver 
        if (from == "fromSender" && callState == "pending") {
            socket.emit("callCancel", { from: userId, to: selectedChat.friendId._id })
        } else if (callState == "accepted") {
            socket.emit("callEnd", { from: userId, to: selectedChat.friendId._id })
        ...
        <>
            ...
            <NavBar />
            {
                ...
                <CallSection onEndCall={handleCallEnd}
                    ...
                    status={callState} mineVid={streamRef} otherVid={receiverStreamRef} />
            }
